...
用 `imgen` CLI 从文字生成图片，或对已有图片做编辑（图生图）。底层是 ChatGPT 付费号的 gpt-image，产出 PNG 文件。
...
imgen 可能装了全局，也可能只在项目里。每次先解析出 `$RUN`：

```bash
if command -v imgen >/dev/null 2>&1; then RUN=imgen; else RUN="node $HOME/workspace_creator/imgen/dist/cli.js"; fi
# dist 不存在（没构建过）则先构建一次
if [ "$RUN" != imgen ] && [ ! -f "$HOME/workspace_creator/imgen/dist/cli.js" ]; then
  (cd "$HOME/workspace_creator/imgen" && npm install && npm run build)
fi
echo "RUN=$RUN"
```

后续命令把 `imgen` 换成解析出的 `$RUN`。

## 文生图

```bash
$RUN "<prompt，英文通常质量更好>" -o <输出.png>
```

## 图生图（编辑已有图）

用户给了图、或上文刚生成过图、或要"改这张"，就走这个，把输入图给 `-i`：

```bash
$RUN "<想怎么改>" -i <输入.png> -o <输出.png>
```
...
3. **把中文需求转成英文 prompt**（gpt-image 对英文响应更好），保留用户要的风格/构图细节。
...
用户：帮我画一张雪地里的红色狐狸，竖版
```bash
$RUN "a red fox sitting in deep snow, cinematic, soft morning light" -s 1024x1536 -o fox.png
```
...
```bash
$RUN "make it nighttime with neon lighting, keep the fox and composition" -i fox.png -o fox-neon.png
```

**Example 3 — 透明贴纸**
用户：做个猫咪贴纸，要透明背景
```bash
$RUN "a cute cartoon cat sticker, bold clean outline" -b transparent -o cat.png
```

## 注意

- **需付费 ChatGPT 号**（imgen 读 `~/.codex/auth.json`）；免费号会 401/403。
- 每张图是一次完整模型推理 + 工具生成 —— 比纯文本慢、更耗账号额度，别无谓地批量跑。
