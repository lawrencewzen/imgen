...
export function loadTokens(codexHome: string): AuthTokens {
  ...
  try {
    ...
    throw new Error(`解析 ${p} 失败：${(error as Error).message}`);
  }
  const tokens = (raw as { tokens?: Partial<AuthTokens> }).tokens;
 ...
 */
export function saveTokensMerged(codexHome: string, tokens: AuthTokens): void {
  const p = authPath(codexHome);
  ...
  for (const [k, v] of Object.entries(tokens)) {
    if (v !== "" && v !== undefined && v !== null) updates[k] = v;
  }
